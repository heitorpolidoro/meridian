...
import {
  ...
  type Agent,
} from "./src/services/AgentRegistryService";
import { BootstrappingService } from "./src/services/BootstrappingService";
import {
  TrackMetadataService,
  type TrackMetadata,
} from "./src/services/TrackMetadataService";
...
vi.mock("express", () => {
  const mExpress = vi.fn(() => mApp) as ReturnType<typeof vi.fn> & {
...
describe("server.ts", () => {
  ...
  describe("processGeminiOutput", () => {
    it("ignores malformed JSON", () => {
      const ctx = makeCtx({});
      const sendACP = vi.fn();
      expect(() => processGeminiOutput("invalid", ctx, sendACP)).not.toThrow();
    });
  ...
  describe("Socket Connection", () => {
    beforeEach(() => {
      ...
      vi.mocked(BootstrappingService.prototype.resolveAgent).mockReturnValue(
        "resolved-a1",
      );
    ...
    it("handles start-session and spawns gemini", () => {
      ...
      const startSessionHandler = vi
        .mocked(socket.on)
        .mock.calls.find((call) => call[0] === "start-session")?.[1];
    ...
    it("handles gemini stdout data", () => {
      ...
      const stdoutOnHandler = vi
        .mocked(mChildProcess.stdout.on)
    ...
    it("handles gemini stderr data", () => {
      ...
      const stderrOnHandler = vi
        .mocked(mChildProcess.stderr.on)
        .mock.calls.find((call) => call[0] === "data")?.[1];
      const spy = vi.spyOn(console, "error").mockReturnValue();
    ...
    it("calls handleInitialize for message with ID 1 and calls sendACP", () => {
      ...
      expect(mChildProcess.stdin.write).toHaveBeenCalledWith(
    ...
    it("handles diretriz event and tests prompt times", () => {
      ...
      const stdoutOnHandler = vi
        ...
        .mock.calls.find((call) => call[0] === "data")?.[1];
      stdoutOnHandler(
        Buffer.from('{"id": 2, "result": {"sessionId": "s1"}}\n'),
      );

      const diretrizHandler = vi
        .mocked(socket.on)
        .mock.calls.find((call) => call[0] === "diretriz")?.[1];
    ...
    describe("New Handlers", () => {
      ...
      it("handles list-dir-contents for a valid directory", () => {
        const handler = vi
          .mocked(socket.on)
          .mock.calls.find((call) => call[0] === "list-dir-contents")?.[1];

        vi.mocked(NodeFileSystem.prototype.exists).mockReturnValue(true);
        vi.mocked(NodeFileSystem.prototype.readDirectory).mockReturnValue([
          "dir1",
          "file1",
          "dir2",
        ]);
        vi.mocked(NodeFileSystem.prototype.isDirectory).mockImplementation(
          (path: string) => path === "/test" || path.includes("dir"),
        );
      ...
      it("handles list-dir-contents when error occurs", () => {
        ...
        vi.mocked(NodeFileSystem.prototype.exists).mockImplementation(() => {
        ...
        expect(spy).toHaveBeenCalledWith(
          expect.stringContaining("Error listing directory"),
        );
        spy.mockRestore();
      });
      ...
      it("handles get-parent-dir", () => {
        const handler = vi
          .mocked(socket.on)
          .mock.calls.find((call) => call[0] === "get-parent-dir")?.[1];
        handler("/test/path");
        expect(socket.emit).toHaveBeenCalledWith(
          "parent-dir",
      ...
      it("handles save-settings", () => {
        const handler = vi
          .mocked(socket.on)
          .mock.calls.find((call) => call[0] === "save-settings")?.[1];
      ...
      it("handles get-agents", () => {
        const handler = vi
          .mocked(socket.on)
          .mock.calls.find((call) => call[0] === "get-agents")?.[1];
      ...
      it("handles save-agents", () => {
        const handler = vi
          .mocked(socket.on)
          .mock.calls.find((call) => call[0] === "save-agents")?.[1];
        const agents = [{ id: "1", name: "A1" }];

        handler(agents);

        expect(AgentRegistryService.prototype.saveAgents).toHaveBeenCalledWith(
          agents,
        );
      ...
      it("handles get-projects", () => {
        const handler = vi
          .mocked(socket.on)
          .mock.calls.find((call) => call[0] === "get-projects")?.[1];
        const projects = [{ id: "p1", name: "Proj 1" }];
        vi.mocked(ProjectService.prototype.listProjects).mockReturnValue(
          projects as Project[],
        );
      ...
      it("handles save-project-config", () => {
        const handler = vi
          .mocked(socket.on)
          .mock.calls.find((call) => call[0] === "save-project-config")?.[1];
        ...
        expect(ProjectService.prototype.saveProjectConfig).toHaveBeenCalledWith(
          "/proj",
          config,
        );
      ...
      it("handles save-project-config with error", () => {
        ...
        const config = { name: "New Proj" };
        vi.mocked(
          ProjectService.prototype.saveProjectConfig,
        ).mockImplementation(() => {
      ...
      it("handles get-file-content", () => {
        const handler = vi
          .mocked(socket.on)
          .mock.calls.find((call) => call[0] === "get-file-content")?.[1];
        vi.mocked(fs.existsSync).mockReturnValue(true);
      ...
      describe("Orchestration Handlers", () => {
        it("handles orchestration:request-transition", () => {
          const handler = vi
            .mocked(socket.on)
            .mock.calls.find(
              (call) => call[0] === "orchestration:request-transition",
            )?.[1];
          const payload = {
            trackId: "t1",
          ...
          expect(
            OrchestrationService.prototype.requestTransition,
          ).toHaveBeenCalledWith("t1", "PLANNING", "start", "USER");
        ...
        it("handles orchestration:request-transition error", () => {
          ...
          expect(socket.emit).toHaveBeenCalledWith("orchestration:error", {
        ...
        it("handles orchestration:update-status", () => {
          const handler = vi
            .mocked(socket.on)
            .mock.calls.find(
              (call) => call[0] === "orchestration:update-status",
            )?.[1];
          const payload = {
            trackId: "t1",
            status: "IN_PROGRESS",
          ...
          expect(
            OrchestrationService.prototype.updateStatus,
          ).toHaveBeenCalledWith("t1", "IN_PROGRESS", "working", "SYSTEM");
          expect(mIO.emit).toHaveBeenCalledWith("tracks", tracks);
        });
        ...
        it("handles orchestration:get-state", () => {
          const handler = vi
            .mocked(socket.on)
            .mock.calls.find(
              (call) => call[0] === "orchestration:get-state",
            )?.[1];
          ...
          expect(socket.emit).toHaveBeenCalledWith("orchestration:state", {
            trackId: "t1",
            state,
          });
        });
  ...
  describe("Module initialization", () => {
    ...
    it("registers fallback route to index.html in production mode", async () => {
      ...
      const getCall = mApp.get.mock.calls.find(
        (call) => call[0].toString() === "/^(?!\\/socket\\.io).+/",
      );
