...
// These stage timings answer one question: how long the agents took. Only
// three statuses can answer it — `in_progress`, `code_review` and `qa_review`
// are the ones the pipeline itself moves a task into as part of dispatching an
// agent (see references/stages.md), so their interval is the agent's run.
//
// Every other status is waiting, not working, and its duration says nothing
// about the agents: `backlog`, `spec_review` and `ready_todo` hold a task until
// an operator runs `meridian:work` again; `spec_approval` is a human gate;
// `blocked` waits on a dependency; `done`/`nope` are terminal and a task can
// sit there forever. Counting any of them makes a stage look slow when nobody
// was slow.
//
// A per-task interval in an untimed status contributes no `totalMs`/`ongoing`
// to its stage entry, and the stage's aggregate carries no avgMs/maxMs/
// topByTime — see aggregateTaskStats and shapeStages below. Visits and token
// attribution are still recorded. Exported so callers/tests reference this
// list instead of re-hardcoding the names.
const UNTIMED_STATUSES = new Set([
    'backlog', 'spec_review', 'spec_approval', 'ready_todo', 'blocked', 'done', 'nope'
]);
...
function aggregateTaskStats(events, now = new Date()) {
    ...
    for (const [taskId, rawEvents] of statusByTask) {
        ...
        for (const iv of intervals) {
            const untimed = UNTIMED_STATUSES.has(iv.stage);
            if (!stages[iv.stage]) {
                stages[iv.stage] = untimed ? { visits: 0 } : { totalMs: 0, visits: 0, ongoing: false };
            }
            stages[iv.stage].visits += 1;
            // An untimed stage never accumulates duration or an ongoing flag —
            // visited-but-unmeasured, regardless of whether this is the task's
            // last, still-open interval.
            if (untimed) continue;
        ...
        for (const [stage, s] of Object.entries(stages)) {
            ...
            if (UNTIMED_STATUSES.has(stage)) continue;
...
function shapeStages(stageTimeEntries, stageTokenEntries, stageAgentEntries, stageVisitEntries) {
    ...
    for (const stage of allStageNames) {
        ...
        const untimed = UNTIMED_STATUSES.has(stage);
...
module.exports = {
    aggregateStats, computeProjectStats, readEventLines, TOP_N, UNTIMED_STATUSES,
