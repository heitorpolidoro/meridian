...
// Statuses a task's time is never measured for: `backlog` (not yet started),
// `done`/`nope` (finished — a task can sit "done" indefinitely without that
// meaning anything about how long the work took). A per-task interval in one
// of these contributes no `totalMs`/`ongoing` to its stage entry, and the
// stage's aggregate carries no avgMs/maxMs/topByTime — see aggregateTaskStats
// and shapeStages below. Exported so callers/tests can reference the same
// list instead of re-hardcoding the three names.
const UNTIMED_STATUSES = new Set(['backlog', 'done', 'nope']);
...
function aggregateTaskStats(events, now = new Date()) {
    ...
    for (const [taskId, rawEvents] of statusByTask) {
        ...
        for (const iv of intervals) {
            ...
            // Untimed stages (backlog/done/nope) never accumulate duration or
            // an ongoing flag — visited-but-unmeasured, regardless of whether
            // this is the task's last, still-open interval.
            if (untimed) continue;
...
// timed or not — it's what keeps an untimed stage (backlog/done/nope) in
