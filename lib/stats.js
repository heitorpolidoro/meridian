...
function aggregateTaskStats(events, now = new Date()) {
    ...
    return { tasks, stageTimeEntries, stageTokenEntries, stageAgentEntries };
}

// Turns three entry-maps (stage -> [{task, ms, ongoing, ...}] / [{task,
// tokens, maxContextTokens, ...}] / [{task, agent, tokens}]) into the final
// `stages` object: avgMs, maxMs, topByTime (TOP_N), avgTokens, maxTokens,
// topByTokens (TOP_N), agents. Extracted verbatim from aggregateStats's
// original final loop so both the single-project and workspace-wide paths
// shape stages identically.
