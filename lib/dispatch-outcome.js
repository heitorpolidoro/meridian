...
// The write side of the rule lib/board.js#queueStallReason reads: when a run
// STARTS for
// a task, whatever the last run said about that same task is no longer the
// truth about it, and must not sit on the card beside a run that is live.
//
// Only that task's outcome goes. A pass refuses several candidates before it
// picks one (see dispatchOnePass), and those refusals explain why the OTHER
// cards did not go — they are still true, and still the only reason those
// cards can show. Returns the entry to store, or null when nothing is left
// worth keeping.
function clearOutcomeFor(entry, taskId) {
    if (!entry || !taskId) return entry || null;

    const refusals = (entry.refusals || []).filter(r => r && r.taskId !== taskId);
    const isAboutThisTask = entry.taskId === taskId;

    if (isAboutThisTask) {
        // The top-level outcome described the task now starting. Any refusal
        // for another task survives on its own.
        return refusals.length ? { refusals } : null;
    }
    if (refusals.length === (entry.refusals || []).length) return entry;
    return { ...entry, refusals };
}

module.exports = { finalEvent, dispatchOutcome, isOAuthContention, OAUTH_CONTENTION_REMEDY, clearOutcomeFor };
