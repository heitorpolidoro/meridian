...
// `installUrl` is the vendor's own installation page, shown when the binary
// is absent. Meridian links rather than installs: installing a CLI is a
// platform-specific `curl | sh`, which does not belong behind a button on a
// local dashboard.
const TOOLS = {
    claude: {
        label: 'Claude Code',
...
// Whether the binary exists at all. Every other state below assumes it does:
// `parsePluginState` reads an absent CLI as "plugin not installed", which is
// true but useless, and readiness reports the spawn error verbatim. The probes
// resolve with the spawn failure rather than throwing, so absence looks like
// ENOENT on both of them — a binary that exists cannot ENOENT, so one probe
// that ran at all settles the question.
function isCliMissing(pluginProbe, readyProbe) {
    const enoent = probe => Boolean(probe) && /ENOENT/.test(String(probe.stderr || ''));
    return enoent(pluginProbe) && enoent(readyProbe);
}
...
function nextAction(cli, state) {
    // Absence outranks everything: nothing can be installed into, updated in
    // or logged into a CLI that is not on this machine.
    if (state && state.present === false) return 'missing';
    if (cli === 'claude' && state && state.ready === false) return 'login';
    if (!state || !state.installed) return 'install';
    // An installed copy that no longer matches the repository is the failure
    // worth surfacing: the agent reads the stale one and nothing says so.
    // Update takes the slot while that is true; uninstall returns once the
    // copy is current again.
    if (state.current === false) return 'update';
    return 'uninstall';
}
...
function commandFor(cli, action, options) {
    ...
    const table = {
        claude: {
            ...
            update: ['claude', 'plugin', 'update', `${PLUGIN_NAME}@${PLUGIN_NAME}`],
            // --claudeai pre-answers the account-type prompt, so the flow needs
            // a terminal but never a keystroke.
            login: ['claude', 'auth', 'login', '--claudeai']
        },
        agy: {
            install: ['agy', 'plugin', 'install', pluginDir],
            // Installing over an existing copy reprocesses it, so update and
            // install are the same command here.
            update: ['agy', 'plugin', 'install', pluginDir],
...
// `claude auth login` opens a browser flow and wants a real terminal; a plain
// spawn hands it pipes. `script` allocates a pty and ships with both macOS and
// Linux, so this costs no dependency — but the two syntaxes differ, and this
// repo runs on macOS while CI is Linux.
function ptyWrap(argv, platform) {
    const os = platform || process.platform;
    if (os === 'darwin') return ['script', '-q', '/dev/null', ...argv];
    return ['script', '-qec', argv.join(' '), '/dev/null'];
}

// Only the login flow needs one. Wrapping the plugin commands too would buy
// nothing and would put their output through a tty layer that rewrites it.
function needsPty(action) {
    return action === 'login';
}

// Where each CLI keeps the copy it installed, so it can be compared with the
// repository. Claude reports its own path in `plugin list --json`; Antigravity
// always uses this one.
const AGY_INSTALL_DIR = path.join(os.homedir(), '.gemini', 'config', 'plugins', PLUGIN_NAME);
...
module.exports = { TOOLS, PROBES, AGY_INSTALL_DIR, parsePluginState, parseReadiness, isCliMissing, nextAction, commandFor, ptyWrap, needsPty };
