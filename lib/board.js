...
// Why a queued task is not moving, or null when there is no known reason —
// either it is not queued, or the queue is simply waiting its turn with
// nothing stuck. An environment-scoped refusal (see refuseDispatch in
// server.js) requeues the task at the front rather than discarding it, on
// purpose: losing queued work because a CLI was logged out would be
// destructive. But that leaves the operator looking at a card that queued
// and then never moved, with nothing on screen explaining why. This reuses
// data the status payload already carries — the project's
// `dispatchBlockedReason` and its `lastRun` — rather than inventing a new
// server field: `lastRun` pins the reason to the exact task it happened to,
// which is more specific than the project-wide `dispatchBlockedReason`
// (e.g. "no dispatch allowlist"), so it wins when both are available.
//
// public/app.js carries an inline copy: the frontend has no module system.
// This file is the source of truth — change both.
