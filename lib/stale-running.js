...
// Meridian itself dispatched is still alive and working it. Otherwise, if
// ANY session — interactive or background — is live in this project's
// directory, a human may be working the task by hand outside the dispatch
// machinery entirely, and there is no way to tell from here whether it is
// this task or another one. Being wrong in that direction costs nothing —
// the task just stays undispatchable one more cycle, exactly as untidy as
// it already was — while wrongly clearing a live session's flag would let a
// second agent land on top of it. So only the narrowest case counts as
// stale: `running: true`, no claiming lock, and no session of any kind in
// the project.
...
function isRunningStale(task, { lock, sessions, projectPath } = {}) {
    ...
    // A session — of any kind — live in this project's directory might be a
    // human working this task by hand. `sessions` is null/undefined when the
    ...
    const hasLiveSessionHere = Array.isArray(sessions) && sessions.some(
        s => s && want !== null && normalisePath(s.cwd) === want
    );
    if (hasLiveSessionHere) return false;
