...
## [MERID-9] Stats stage table: agent column replaces task count — 2026-09-07
- `formatStageAgentCell`'s `title` attribute only escapes `"` (`.replace(/"/g, '&quot;')`), and the visible `label`/`suffix` text is interpolated into `innerHTML` with no escaping at all. If an `agent` string ever contained `<`/`>`/`&`, it would be parsed as markup rather than displayed literally. This is not a regression introduced by this spec, though: I confirmed `renderStatsPanel`'s existing code (stage `name`, `r.task`, `r.stage`, `r.project`) is already interpolated into `innerHTML` the same way, with no `escapeHtml` helper anywhere in `app.js` and no escaping policy documented in `AGENTS.md`. The spec's treatment is consistent with the codebase's established (if imperfect) convention, so I'm not blocking on it — but if the team wants to close this gap, doing it for all of these fields at once (not just `agent`) would be the coherent fix rather than a one-off in this task.
- No tie-break rule is specified for two agents with exactly equal `totalOutputTokens` in a stage (which agent is `agents[0]`, i.e. the "dominant" one shown without the `+N` label). `Array.prototype.sort` is stable in current Node/V8, so the outcome is deterministic (insertion order of first-seen agent in that stage) even though the spec doesn't say so explicitly. Not blocking — behavior is well-defined by the given code, just undocumented.


## [MERID-9] Stats stage table: agent column (code review r1) — 2026-09-07

- No test exercises the new `escapeHtml`/HTML-injection path in `formatStageAgentCell` (e.g. an agent name containing `<`/`"`/`&`). Not required by the spec's testing section (this codebase has no DOM runner, per its own out-of-scope note), and the implementation is correct and in fact stricter than the spec's own pseudocode (which left the cell label unescaped) — but a `formatStageAgentCell`/`formatAgentName` unit test asserting the escaped output would be cheap insurance if this file ever grows a DOM test runner.
- `test/api-tasks.test.js`'s `'POST rejects a parent naming a task id that does not exist'` failed once under the full `npm test` run (`Server on port 3659 did not respond after 5166ms`) but passed cleanly when `test/api-tasks.test.js` was run in isolation immediately after. This is unrelated to any file touched by this diff (parent-validation logic, not stats) and looks like a port-contention/timing flake in the full-suite run rather than a regression introduced here — flagging for awareness, not blocking this review.


## [MERID-9] Stats stage table: agent column (QA) — 2026-09-07
