...
io.on('connection', (socket) => {
    ...
    const ctx: GeminiContext = {
        globalContent: '', agentInstructions: '', rootDir: '',
        socket, telemetryCollector,
        setSessionId: (id: string) => { sessionId = id; },
        setPromptStartTime: (time: number | null) => { promptStartTime = time; },
    ...
    socket.on('start-session', () => {
        ...
        ctx.rootDir = rootDir;
        ctx.globalContent = fs.existsSync(path.join(meridianDir, 'core/global.md')) ? fs.readFileSync(path.join(meridianDir, 'core/global.md'), 'utf8') : '';
        ctx.agentInstructions = agents.map((a: any) => `${a.name.toUpperCase()} (${a.role}): ${bootstrappingService.resolveAgent(a.id)}`).join('\n\n---\n\n');

        gemini = spawn(GEMINI_CMD, ['--experimental-acp', '--output-format', 'stream-json', '--resume', 'latest', '-y', '--extensions', ''], {
            stdio: ['pipe', 'pipe', 'pipe'], cwd: rootDir, env: { ...process.env, PYTHONUNBUFFERED: '1' }
        });

        gemini.stdout?.on('data', (data: Buffer) => handleGeminiStream(data, { ...ctx, gemini }));
        socket.emit('status', 'Initializing...');
        sendACP({ jsonrpc: "2.0", id: 1, method: "initialize", params: { protocolVersion: 0, clientInfo: { name: "meridian-ai", version: "1.0" }, capabilities: {} } });
    });
