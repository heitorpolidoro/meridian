...
describe("BootstrappingService", () => {
  ...
  beforeEach(() => {
    ...
    mockFs.writeFile(
      path.join(rootDir, ".meridian/core/global.md"),
      "# Global Standards",
    );
    mockFs.writeFile(
      path.join(rootDir, ".meridian/core/backend.md"),
      "# Backend Standards",
    );

    mockFs.writeFile(
      path.join(rootDir, ".meridian/roles/architect.md"),
      `# Role: Architect
@../core/backend.md
Some instructions.`,
    );
    ...
    mockFs.writeFile(
      path.join(rootDir, ".gemini/agents/architect.md"),
      `---
name: architect
---
# Stub
@../../.meridian/roles/architect.md`,
  ...
  it("should throw error if global standards file is missing in resolve()", () => {
    ...
    expect(() => service.resolve("architect")).toThrow(
      "Global standards file not found",
    );
  });
...
# Role: Architect`,
  ...
  it("should handle missing linked files gracefully by including error message in content", () => {
    mockFs.writeFile(
      path.join(rootDir, ".meridian/roles/missing.md"),
      "@../core/notfound.md",
    );
    mockFs.writeFile(
  ...
  it("should resolve deep recursion", () => {
    mockFs.writeFile(
      path.join(rootDir, ".meridian/core/base.md"),
      "# Base Standards",
    );
    mockFs.writeFile(
      path.join(rootDir, ".meridian/core/backend.md"),
