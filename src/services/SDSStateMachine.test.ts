...
describe("SDSStateMachine", () => {
  describe("validateTransition", () => {
    it.each([
      ...
      "transition from %s to %s should be valid: %s",
      (from, to, expected, errorMsg?) => {
        const result = SDSStateMachine.validateTransition(
          from as unknown,
          to as unknown,
        );
  ...
  describe("getNextPhase", () => {
    it.each([
      ...
      expect(
        SDSStateMachine.getNextPhase(
          current as Parameters<typeof SDSStateMachine.getNextPhase>[0],
        ),
      ).toBe(expected);
    });
  });
